...
def Generate_Brain():
    ...
    """pyrosim.Send_Synapse(sourceNeuronName=0, targetNeuronName=4, weight=0.0) # Why would the torso move?
    pyrosim.Send_Synapse(sourceNeuronName=0, targetNeuronName=3, weight=0.0)
    pyrosim.Send_Synapse(sourceNeuronName=1, targetNeuronName=4, weight=1.0)
    pyrosim.Send_Synapse(sourceNeuronName=1, targetNeuronName=3, weight=3.0) # Nothing else has to refer to these
    ...
    for n in [0, 1, 2]:
        for k in [3, 4]:
            pyrosim.Send_Synapse(sourceNeuronName=n, targetNeuronName=k, weight=random.uniform(-1, 1))
    pyrosim.End()
...
def Generate_Body():
    ...
    pyrosim.Send_Joint(name="Torso_Frontleg", child="Frontleg", parent="Torso", type="revolute", position=[0.5,0,1])
